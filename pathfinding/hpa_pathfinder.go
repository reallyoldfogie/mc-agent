...
func NewHPAPathFinder(world models.World, shapeMgr models.BlockShapeManager, lowLevelPathfinder models.PathFinder, clusterSize int, logger *slog.Logger) models.PathFinder {
	logger = utils.SafeLogger(logger)
	builder := NewHPABuilder(world, shapeMgr, lowLevelPathfinder, clusterSize, logger)
...
func NewHPAPathFinderWithAStar(world models.World, shapeMgr models.BlockShapeManager, clusterSize int, logger *slog.Logger) models.PathFinder {
	lowLevel := NewAStarPathFinder(world, shapeMgr, logger)
	return NewHPAPathFinder(world, shapeMgr, lowLevel, clusterSize, logger)
}
...
func NewHPAPathFinderWithBidirectional(world models.World, shapeMgr models.BlockShapeManager, clusterSize int, logger *slog.Logger) models.PathFinder {
	lowLevel := NewBidirAStarPathFinder(world, shapeMgr, logger)
	return NewHPAPathFinder(world, shapeMgr, lowLevel, clusterSize, logger)
}
...
func NewHPAPathFinderWithEPEAStar(world models.World, shapeMgr models.BlockShapeManager, clusterSize int, logger *slog.Logger) models.PathFinder {
	lowLevel := NewEPEAStarPathFinder(world, shapeMgr, logger)
	return NewHPAPathFinder(world, shapeMgr, lowLevel, clusterSize, logger)
}
...
func (hpa *hpaPathFinder) FindPath(ctx context.Context, start, goal models.V3, maxSteps int) (resultPath *Path, err error) {
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Finding path from (%.0f,%.0f,%.0f) to (%.0f,%.0f,%.0f)",
	...
	if dist <= float64(hpa.clusterSize) {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Short distance %.1f (<= clusterSize %d), using low-level pathfinding",
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Start cluster: %s, Goal cluster: %s", startClusterID.String(), goalClusterID.String()))
	...
	if startClusterID == goalClusterID {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Same cluster, using low-level pathfinding"))
		return hpa.lowLevelPathfinder.FindPath(ctx, start, goal, maxSteps)
	}
	if clustersAdjacent(startClusterID, goalClusterID) {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Adjacent clusters, using low-level pathfinding"))
		return hpa.lowLevelPathfinder.FindPath(ctx, start, goal, maxSteps)
	}
	...
	if len(startNodes) == 0 {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Cannot connect start to abstract graph, falling back to low-level pathfinding"))
		return hpa.lowLevelPathfinder.FindPath(ctx, start, goal, maxSteps)
	}
	if len(goalNodes) == 0 {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Cannot connect goal to abstract graph, falling back to low-level pathfinding"))
		return hpa.lowLevelPathfinder.FindPath(ctx, start, goal, maxSteps)
	}

	filterStartEdges := filterTempNodeEdges(hpa.logger, startNodes, "start")
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Inserted start (%d connections) and goal (%d connections to entrances)",
	...
	if filterStartEdges || len(filteredGoalEntrances) > 0 {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Filtered entrance candidates: startFiltered=%t goalFiltered=%t",
	...
	for i, goalEntrance := range goalEntrances {
		...
		if abstractPath == nil {
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] No abstract path to goal entrance %d/%d at (%.0f,%.0f,%.0f)",
				i+1, len(goalEntrances), goalEntrance.GetPosition().X, goalEntrance.GetPosition().Y, goalEntrance.GetPosition().Z))
			continue
		}

		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Found abstract path to goal entrance %d/%d with %d edges", i+1, len(goalEntrances), len(abstractPath)))
		...
		if refinedPath != nil {
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Successfully refined path through goal entrance %d/%d", i+1, len(goalEntrances)))
			break // Success!
		}

		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Failed to refine path through goal entrance %d/%d, trying next entrance", i+1, len(goalEntrances)))
	}
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Refined path: %d steps, cost=%.2f, time=%.0fms",
		len(refinedPath.Steps), refinedPath.TotalCost, refinedPath.SearchTime))

	// Log first 10 steps to diagnose direction issues
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Refined path first 10 steps:"))
	for i := 0; i < len(refinedPath.Steps) && i < 10; i++ {
		step := refinedPath.Steps[i]
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*]   Step %d: %s to (%.1f, %.2f, %.1f)",
	...
	if err := hpa.validatePath(normalized); err != nil {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Refined path validation failed: %v", err))
...
func (hpa *hpaPathFinder) insertNode(ctx context.Context, pos models.V3) ([]*AbstractNode, func()) {
	...
	for _, entrance := range cluster.Entrances {
		...
		if bestPath != nil {
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Best path to entrance: pos=(%.0f,%.0f,%.0f), cost=%.2f (tried %d positions)",
			...
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Failed to find path to any position in entrance group (tried %d positions)", len(positionsToTry)))
		}
	}
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] insertNode at (%.0f,%.0f,%.0f): %d entrances, %d candidates after cost limit",
	...
	for i, candidate := range candidates {
		...
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*]   candidate %d: entrance=(%.0f,%.0f,%.0f) cost=%.2f",
	...
	if len(tempNode.Edges) == 0 {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Warning: insertNode at (%.0f,%.0f,%.0f) found NO reachable entrances in cluster %s",
			pos.X, pos.Y, pos.Z, clusterID.String()))
		return nil, cleanup
	}

	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] insertNode at (%.0f,%.0f,%.0f): connected to %d entrances",
...
func (hpa *hpaPathFinder) refinePathThroughEntrance(ctx context.Context, abstractPath []*AbstractEdge, start, goal models.V3, goalTempNode *AbstractNode) *Path {
		...
		if edge.Path != nil {
			...
			if len(edge.Path.Steps) > 0 {
				...
				if !positionsEqual(currentPos, firstStep) {
					utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Gap detected in abstract edge %d/%d: currentPos=(%.0f,%.0f,%.0f) but cached path starts at (%.0f,%.0f,%.0f)",
					...
					if err != nil || !bridgePath.Found {
						utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Failed to bridge gap in abstract edge %d/%d from (%.0f,%.0f,%.0f) to (%.0f,%.0f,%.0f): %v",
							i+1, len(abstractPath), currentPos.X, currentPos.Y, currentPos.Z, firstStep.X, firstStep.Y, firstStep.Z, err))
						return nil
					}

					utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Created bridge path for abstract edge %d/%d with %d steps, cost=%.2f",
			...
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] After abstract edge %d/%d (cached): currentPos=(%.0f,%.0f,%.0f), entrancePos=(%.0f,%.0f,%.0f)",
			...
			if err != nil || !path.Found {
				utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Failed to refine abstract edge %d/%d from (%.0f,%.0f,%.0f) to (%.0f,%.0f,%.0f) with maxSteps=%d",
			...
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] After abstract edge %d/%d (computed): currentPos=(%.0f,%.0f,%.0f), entrancePos=(%.0f,%.0f,%.0f)",
				i+1, len(abstractPath), currentPos.X, currentPos.Y, currentPos.Z, nextPos.X, nextPos.Y, nextPos.Z))
		}
	}
	...
	for _, edge := range goalTempNode.Edges {
		if edge.To.GetPosition() == entrancePos && edge.Path != nil {
			...
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Found cached path from goal to entrance:"))
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*]   Cached path: StartPos=(%.0f,%.0f,%.0f) GoalPos=(%.0f,%.0f,%.0f) Steps=%d",
				...
				len(edge.Path.Steps)))
			if len(edge.Path.Steps) > 0 {
				utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*]   First step: to (%.0f,%.0f,%.0f), Last step: to (%.0f,%.0f,%.0f)",
	...
	if finalSegment == nil {
		...
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] No cached final segment, computing from (%.0f,%.0f,%.0f) to goal (%.0f,%.0f,%.0f)",
		...
		if err != nil || !finalSegment.Found {
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Failed to find final segment from entrance (%.0f,%.0f,%.0f) to goal (%.0f,%.0f,%.0f)",
				currentPos.X, currentPos.Y, currentPos.Z, goal.X, goal.Y, goal.Z))
			return nil
		}
	} else {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Using cached (reversed) final segment: %d steps", len(finalSegment.Steps)))
	...
	if finalSegment != nil && len(finalSegment.Steps) > 0 {
		...
		if !positionsEqual(currentPos, firstStep) {
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Gap detected: currentPos=(%.0f,%.0f,%.0f) but finalSegment starts at (%.0f,%.0f,%.0f)",
			...
			bridgePath, err := hpa.lowLevelPathfinder.FindPath(ctx, currentPos, firstStep, bridgeMaxSteps)
			if err != nil || !bridgePath.Found {
				utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Failed to bridge gap from (%.0f,%.0f,%.0f) to (%.0f,%.0f,%.0f): %v",
					currentPos.X, currentPos.Y, currentPos.Z, firstStep.X, firstStep.Y, firstStep.Z, err))
				return nil
			}

			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Created bridge path with %d steps, cost=%.2f",
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Raw path before normalization: %d steps", len(allSteps)))
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] First 10 steps:"))
	...
	if len(allSteps) > 10 {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Last 10 steps:"))
	...
	if err != nil {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] RefinePath normalization failed: %v", err))
		return nil
	}
	if err := hpa.validatePath(normalized); err != nil {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] RefinePath validation failed: %v", err))
		return nil
	}
	return normalized
}
...
func (hpa *hpaPathFinder) validatePath(path *Path) error {
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[hpaPathFinder.validatePath] validating path: %s", path.LogDetails(true)))
	prev := path.StartPos
...
func (hpa *hpaPathFinder) buildClusterRegion(startClusterID, goalClusterID ClusterID) {
	...
	if clusterCount > maxRegionClusters {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Skipping region build for %d clusters (max %d); abstract graph may be incomplete",
...
func filterTempNodeEdges(logger *slog.Logger, nodes []*AbstractNode, label string) bool {
	changed := false
	...
	if changed {
		logger.Debug(fmt.Sprintf("[HPA*] Filtered temp node edges for %s node(s)", label))
	}
	return changed
}
...
func (hpa *hpaPathFinder) logAbstractPath(abstractPath []*AbstractEdge) {
	if len(abstractPath) == 0 {
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Abstract path: <empty>"))
		return
	}
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Abstract path edges:"))
	for i, edge := range abstractPath {
		pos := edge.To.GetPosition()
		utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*]   edge %d: to (%.0f,%.0f,%.0f) cost=%.2f",
...
func (hpa *hpaPathFinder) refinePath(ctx context.Context, abstractPath []*AbstractEdge, start, goal models.V3) *Path {
	...
	for i, edge := range abstractPath {
		...
		if edge.Path != nil {
			...
			if err != nil || !path.Found {
				utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Warning: Failed to refine edge %d/%d from (%.0f,%.0f,%.0f) to (%.0f,%.0f,%.0f)",
					i+1, len(abstractPath), currentPos.X, currentPos.Y, currentPos.Z, nextPos.X, nextPos.Y, nextPos.Z))
				continue
			}
	...
	if currentPos != goal {
		...
		if err != nil || !finalPath.Found {
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Failed to find path from last entrance (%.0f,%.0f,%.0f) to goal (%.0f,%.0f,%.0f): %v",
...
func (hpa *hpaPathFinder) BuildClustersInRegionWithContext(ctx context.Context, minPos, maxPos models.V3) error {
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Building %d clusters in region %s to %s (parallel, %d workers)",
	...
	for _, id := range clusterIDs {
		...
		select {
		case <-ctx.Done():
			utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Cluster building cancelled after %d/%d clusters", clustersBuilt, totalClusters))
			return ctx.Err()
		default:
		}
	...
	utils.SafeLogger(hpa.logger).Debug(fmt.Sprintf("[HPA*] Built %d clusters in %v (%.1f clusters/sec)",
