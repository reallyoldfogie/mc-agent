...
func TestEntityHandler_ParseMoveEntityPos(t *testing.T) {
	...
	// Verify packet ID is correct for 1.21.5
	if pkt.PacketID() != 46 {
		t.Errorf("Expected packet ID 46, got %d", pkt.PacketID())
	}
...
func TestEntityHandler_ParseMoveEntityPosRot(t *testing.T) {
	...
	// Verify packet ID is correct for 1.21.5
	if pkt.PacketID() != 47 {
		t.Errorf("Expected packet ID 47, got %d", pkt.PacketID())
	}
...
func TestEntityHandler_ParseTeleportEntity(t *testing.T) {
	...
	// Verify packet ID is correct for 1.21.5
	if pkt.PacketID() != 118 {
		t.Errorf("Expected packet ID 118, got %d", pkt.PacketID())
	}
...
func TestEntityHandler_ParseRemoveEntities(t *testing.T) {
	...
	// Verify packet ID is correct for 1.21.5
	if pkt.PacketID() != 70 {
		t.Errorf("Expected packet ID 70, got %d", pkt.PacketID())
	}
...
func TestEntityHandler_ParseAddEntity_PacketIDs(t *testing.T) {
	// Verify packet IDs match expected values for 1.21.5
	tests := []struct {
		name       string
		createPkt  func() interface{ PacketID() int32 }
		expectedID int32
	}{
		{"SpawnEntity", func() interface{ PacketID() int32 } { return cb.NewSpawnEntity() }, 1},
		{"RelEntityMove", func() interface{ PacketID() int32 } { return cb.NewRelEntityMove() }, 46},
		{"EntityMoveLook", func() interface{ PacketID() int32 } { return cb.NewEntityMoveLook() }, 47},
		{"EntityTeleport", func() interface{ PacketID() int32 } { return cb.NewEntityTeleport() }, 118},
		{"EntityDestroy", func() interface{ PacketID() int32 } { return cb.NewEntityDestroy() }, 70},
		{"EntityStatus", func() interface{ PacketID() int32 } { return cb.NewEntityStatus() }, 30},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			pkt := tc.createPkt()
			if pkt.PacketID() != tc.expectedID {
				t.Errorf("%s: expected packet ID %d, got %d", tc.name, tc.expectedID, pkt.PacketID())
			}
		})
	}
}
