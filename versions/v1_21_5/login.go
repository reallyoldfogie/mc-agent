...
// loginHandler implements common.LoginHandler for 1.21.5.
...
func (l *loginHandler) SendLoginStart(conn common.PacketWriter, username string, uuid [16]byte) error {
	...
	log.Printf("[v1.21.5 Login] SendLoginStart: username=%s uuid=%x", username, uuid)
...
func (l *loginHandler) SendEncryptionResponse(conn common.PacketWriter, sharedSecret, verifyToken []byte) error {
	...
	log.Printf("[v1.21.5 Login] SendEncryptionResponse: sharedSecretLen=%d verifyTokenLen=%d",
...
func (l *loginHandler) SendLoginAcknowledged(conn common.PacketWriter) error {
	...
	log.Printf("[v1.21.5 Login] SendLoginAcknowledged")
...
func (l *loginHandler) ParseLoginSuccess(p pk.Packet) (username string, uuid [16]byte, err error) {
	...
	log.Printf("[v1.21.5 Login] ParseLoginSuccess: username=%s uuid=%x", username, uuid)
...
func (l *loginHandler) ParseEncryptionRequest(p pk.Packet) (serverID string, publicKey, verifyToken []byte, err error) {
	...
	log.Printf("[v1.21.5 Login] ParseEncryptionRequest: serverID=%s publicKeyLen=%d verifyTokenLen=%d",
