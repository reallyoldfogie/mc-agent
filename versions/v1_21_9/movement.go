...
func (m *movementHandler) SendPosition(conn models.PacketWriter, x, y, z float64, onGround bool) error {
	...
	log.Printf("[v1.21.9 Movement] SendPosition: (%.2f, %.2f, %.2f) onGround=%v", x, y, z, onGround)
...
func (m *movementHandler) SendPositionAndRotation(conn models.PacketWriter, x, y, z float64, yaw, pitch float32, onGround bool) error {
	...
	log.Printf("[v1.21.9 Movement] SendPositionAndRotation[ID=%d (0x%X)]: (%.2f, %.2f, %.2f) yaw=%.2f pitch=%.2f onGround=%v",
...
func (m *movementHandler) SendPlayerCommand(conn models.PacketWriter, entityID, actionID int32) error {
	...
	log.Printf("[v1.21.9 Movement] SendPlayerCommand: entityID=%d actionID=%d -> %s", entityID, actionID, actionStr)
...
func (m *movementHandler) sendPlayerInput(conn models.PacketWriter) error {
	...
	log.Printf("[v1.21.9 Movement] SendPlayerInput: shift=%v sprint=%v", m.isSneaking, m.isSprinting)
...
func (m *movementHandler) SendTeleportConfirm(conn models.PacketWriter, teleportID int32) error {
	...
	log.Printf("[v1.21.9 Movement] SendTeleportConfirm: teleportID=%d", teleportID)
...
func (m *movementHandler) SendPlayerAbilities(conn models.PacketWriter, flags byte) error {
	...
	log.Printf("[v1.21.9 Movement] SendPlayerAbilities: flags=%d", flags)
...
func (m *movementHandler) SendMoveVehicle(conn models.PacketWriter, x, y, z float64, yaw, pitch float32, onGround bool) error {
	...
	log.Printf("[v1.21.9 Movement] SendMoveVehicle: (%.2f, %.2f, %.2f) yaw=%.2f pitch=%.2f onGround=%v", x, y, z, yaw, pitch, onGround)
...
func (m *movementHandler) SendVehicleInput(conn models.PacketWriter, forward, backward, left, right, jump, sneak bool) error {
	...
	log.Printf("[v1.21.9 Movement] SendVehicleInput: forward=%v backward=%v left=%v right=%v jump=%v sneak=%v",
...
func (m *movementHandler) SendPlayerCommandWithParam(conn models.PacketWriter, entityID, actionID, jumpBoost int32) error {
	...
	log.Printf("[v1.21.9 Movement] SendPlayerCommandWithParam: entityID=%d actionID=%d -> %s jumpBoost=%d", entityID, actionID, actionStr, jumpBoost)
