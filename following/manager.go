...
func (fm *followManager) Start(targetPlayerName string) error {
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Start called, target='%s'", fm.getFollowerName(), targetPlayerName))
	...
	for {
		...
		case <-ticker.C:
			if err := fm.update(); err != nil {
				utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Follow loop error: %v", fm.getFollowerName(), err))
...
func (fm *followManager) update() error {
	...
	if fm.lastRecalcStatsLog.IsZero() || time.Since(fm.lastRecalcStatsLog) >= 1*time.Second {
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] RecalcStats ticks=%d calls=%d skips={inactive:%d target_lost:%d arrived:%d transition:%d uninit:%d path_nil:%d progress:%d}",
			fm.getFollowerName(),
			fm.statTickTotal,
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] current following state: %s", fm.getFollowerName(), fm.state))
	...
	if !fm.lastTargetPosTime.IsZero() {
		...
		if lastDist > maxExpectedMovement && lastDist > 2.0 {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Target teleported %.1f blocks from (%.2f %.2f %.2f) to (%.2f %.2f %.2f) in %.2fs (expected max: %.1f)",
				fm.getFollowerName(),
				lastDist,
				fm.lastTargetPos.x, fm.lastTargetPos.y, fm.lastTargetPos.z,
				targetX, targetY, targetZ,
	...
	if len(fm.targetPosHistory) >= 2 {
		...
		if fm.targetWasStationary && velocityMagnitude > 0.5 {
			...
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Target started moving (velocity %.2f b/s) - clearing stale position history and forcing immediate recalc",
	...
	if fm.lastDetailedLog.IsZero() || time.Since(fm.lastDetailedLog) > 5*time.Second {
		...
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] === Follow State === Distance: %.2f | Velocity: %.2f b/s | State: %s | Last path: %.1fs ago | Predictive: %s",
	...
	if !fm.lastTransitionTime.IsZero() && time.Since(fm.lastTransitionTime) < chaseWindow {
		...
		if arrivedGate {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Arrived gate SUPPRESSED during chase window (%.0fms) dist=%.2f, moved=%.2f", fm.getFollowerName(), chaseWindow.Seconds()*1000, distance, targetMovedDistance))
		}
		arrivedGate = false
	}
	...
	if arrivedGate && transitionedToMoving {
		if fm.state == models.StateArrived {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Arrived gate OFF (transition detected) dist=%.2f, moved=%.2f>%.2f", fm.getFollowerName(), distance, targetMovedDistance, fm.config.TargetStillDistance))
			fm.state = models.StateFollowingPath
		}
		...
		if fm.state != models.StateArrived {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Arrived gate ON dist=%.2f<=%.2f, still=%.2f<=%.2f", fm.getFollowerName(), distance, fm.config.StopDistance, targetMovedDistance, fm.config.TargetStillDistance))
		...
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Arrived gate OFF dist=%.2f>%.2f or moved=%.2f>%.2f", fm.getFollowerName(), distance, fm.config.StopDistance, targetMovedDistance, fm.config.TargetStillDistance))
		fm.state = models.StateFollowingPath
	}
...
func (fm *followManager) monitorPathExecution(botX, botY, botZ, targetX, targetY, targetZ float64) error {
	...
	if !fm.lastProgressCheck.IsZero() && now.Sub(fm.lastProgressCheck) > 2*time.Second {
		...
		if botDistMoved < 0.5 && targetDistMoved > 2.0 {
			fm.statSkipProgressRecalc++
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Stuck following outdated path: bot moved %.2f, target moved %.2f - forcing recalc",
	...
	if fm.currentPath == nil {
		fm.statSkipPathNil++
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] monitor: currentPath is nil, forcing path calculation", fm.getFollowerName()))
		return fm.calculateNewPath(botX, botY, botZ, targetX, targetY, targetZ)
	}

	if needsRecalc {
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] monitor: needsRecalc=true (dist=%.2f, sinceLastPath=%.2fs)", fm.getFollowerName(), fm.calculateDistance(botX, botY, botZ, targetX, targetY, targetZ), time.Since(fm.lastPathTime).Seconds()))
		return fm.calculateNewPath(botX, botY, botZ, targetX, targetY, targetZ)
	}
	...
	if time.Since(fm.lastPathTime) > fm.config.StuckThreshold {
		...
		if fm.currentPath != nil && len(fm.currentPath.Steps) > 0 {
			...
			if currentDist > initialDist {
				utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Distance to target increased (%.2f -> %.2f) - forcing recalc",
...
func (fm *followManager) followWithMoveTowards(botX, botY, botZ, targetX, targetY, targetZ float64) error {
	...
	if distance > fm.config.SprintDistance {
		...
		if err := fm.movementExecutor.StartSprinting(); err != nil {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] StartSprinting error: %v", fm.getFollowerName(), err))
		}
		// Make sure not sneaking while sprinting
		if err := fm.movementExecutor.StopSneaking(); err != nil {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] StopSneaking error: %v", fm.getFollowerName(), err))
		}
	} else if distance < fm.config.SneakDistance {
		// Very close to target - sneak
		if err := fm.movementExecutor.StopSprinting(); err != nil {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] StopSprinting error: %v", fm.getFollowerName(), err))
		}
		if err := fm.movementExecutor.StartSneaking(); err != nil {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] StartSneaking error: %v", fm.getFollowerName(), err))
		}
	} else {
	...
	if err := fm.movementExecutor.LookAt(targetX, targetY, targetZ, true); err != nil {
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] LookAt target player error: %v", fm.getFollowerName(), err))
	}
	...
	if err != nil {
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Movement error: %v", fm.getFollowerName(), err))
		return err
	}
...
func (fm *followManager) shouldRecalculatePath(targetX, targetY, targetZ float64) bool {
	...
	if distMoved > threshold {
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Path recalc needed: target moved %.2f blocks (from %.2f,%.2f,%.2f to %.2f,%.2f,%.2f)",
	...
	if fm.lastRecalcDecisionLog.IsZero() || time.Since(fm.lastRecalcDecisionLog) >= 1*time.Second {
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] No recalc: target moved %.2f ≤ %.2f since last path (from %.2f,%.2f,%.2f to %.2f,%.2f,%.2f)",
...
func (fm *followManager) calculateNewPath(botX, botY, botZ, targetX, targetY, targetZ float64) error {
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Calculating new path from (%.2f %.2f %.2f) to (%.2f %.2f %.2f)", fm.getFollowerName(), botX, botY, botZ, targetX, targetY, targetZ))
	...
	if velocityMagnitude > 1.0 { // Target is moving (>1 block/sec)
		...
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Predictive targeting: velocity=%.2f b/s, predicting %.2fs ahead to (%.2f, %.2f, %.2f)",
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Pathfinding from (%.2f, %.2f, %.2f) to (%.2f, %.2f, %.2f) [target distance: %.1f blocks from player]",
		fm.getFollowerName(), start.X, start.Y, start.Z, goal.X, goal.Y, goal.Z, fm.config.TargetDistance))

	line := utils.Line(start, goal)
	lineLength := len(line)

	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Pathfinding straight line distance from (%.2f, %.2f, %.2f) to (%.2f, %.2f, %.2f) = %d",
	...
	case <-time.After(fm.config.PathfindingTimeout):
		...
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Pathfinding timed out after %v", fm.getFollowerName(), fm.config.PathfindingTimeout))
		return fmt.Errorf("pathfinding timed out after %v", fm.config.PathfindingTimeout)
	}

	if err != nil {
		fm.state = models.StateStuck
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Pathfinding failed: %v", fm.getFollowerName(), err))
		return err
	}

	if path == nil || !path.Found {
		fm.state = models.StateStuck
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] no path found", fm.getFollowerName()))
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] lastPathTargetPos updated to (%.2f, %.2f, %.2f)", fm.getFollowerName(), fm.lastPathTargetPos.X, fm.lastPathTargetPos.Y, fm.lastPathTargetPos.Z))
	...
	if fm.pathExecutor != nil {
		if err := fm.pathExecutor.SetPath(path); err != nil {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Failed to set path on executor: %v", fm.getFollowerName(), err))
			return err
		}
	}

	// Log detailed path information
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s]  %s", fm.getFollowerName(), path.LogSummary()))
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s]  Path details:\n%s", fm.getFollowerName(), path.LogDetails(true)))
...
func (fm *followManager) handleStuck(botX, botY, botZ, targetX, targetY, targetZ float64) error {
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Stuck detected (attempt %d/%d) at position (%.1f, %.1f, %.1f)",
	...
	if fm.stuckAttempts >= fm.config.MaxStuckAttempts {
		...
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Exhausted recovery attempts, stopping follow", fm.getFollowerName()))
		fm.active = false
		fm.state = models.StateIdle
	...
	if recoveryErr != nil {
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Recovery attempt %d failed: %v", fm.getFollowerName(), fm.stuckAttempts, recoveryErr))
	}
...
func (fm *followManager) tryJumpRecovery(botX, botY, botZ float64) error {
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Attempting jump recovery at (%.1f, %.1f, %.1f)", fm.getFollowerName(), botX, botY, botZ))
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Jump recovery completed", fm.getFollowerName()))
	return nil
}
...
func (fm *followManager) trySidewaysRecovery(botX, botY, botZ, targetX, targetY, targetZ float64) error {
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Attempting sideways recovery at (%.1f, %.1f, %.1f)", fm.getFollowerName(), botX, botY, botZ))
	...
	for attempt := range 4 {
		...
		if err == nil {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Sideways movement succeeded to (%.1f, %.1f, %.1f)", fm.getFollowerName(), sidewaysX, botY, sidewaysZ))
			break
		}
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Sideways movement attempt %d failed: %v", fm.getFollowerName(), attempt+1, err))
	}
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Sideways recovery completed", fm.getFollowerName()))
	return nil
}
...
func (fm *followManager) tryAlternateDestination(botX, botY, botZ, targetX, targetY, targetZ float64) error {
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Attempting alternate destination recovery", fm.getFollowerName()))
	...
	for i, offset := range alternateOffsets {
		...
		utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Trying alternate destination %d: (%f, %f, %f)", fm.getFollowerName(), i+1, goal.X, goal.Y, goal.Z))
		...
		if err == nil && path.Found {
			utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Found alternate path with %d steps", fm.getFollowerName(), len(path.Steps)))
			fm.currentPath = path
			fm.lastPathTime = time.Now()
			if fm.pathExecutor != nil {
				if err := fm.pathExecutor.SetPath(path); err != nil {
...
func (fm *followManager) tryBackupRecovery(botX, botY, botZ, targetX, targetY, targetZ float64) error {
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Attempting backup recovery", fm.getFollowerName()))
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Moving backwards from (%.1f, %.1f, %.1f) to (%.1f, %.1f, %.1f)",
	...
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] Backup recovery completed", fm.getFollowerName()))
	return nil
}
...
func (fm *followManager) tryClearPathRecovery() error {
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("Clearing path for fresh recalculation"))
...
func (fm *followManager) handleTargetLost(reason string) error {
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("[FollowManager %s] [TARGET_LOST] transitioning following state from %s to %s", fm.getFollowerName(), fm.state.String(), models.StateLost.String()))
	fm.state = models.StateLost
	fm.sendChatMessage(fmt.Sprintf("Lost target %s: %s", fm.targetName, reason))
	utils.SafeLogger(fm.logger).Debug(fmt.Sprintf("Target lost: %s", reason))
	fm.targetEntityID = 0
	fm.lastTargetPosTime = time.Time{}
	fm.clearPath()
	// fm.active = false
