...
func handleUpdateHealthPacket(c *Client, p pk.Packet) error {
	...
	oldHealth := c.Health
	newHealth := float32(pkt.Health)
	c.Health = newHealth

	oldFood := c.Food
	newFood := int32(pkt.Food)
	c.Food = newFood

	oldFoodSaturation := c.FoodSaturation
	newFoodSaturation := float32(pkt.FoodSaturation)
	c.FoodSaturation = newFoodSaturation

	if c.Events.HealthChange != nil {
		if err := c.Events.HealthChange(oldHealth, newHealth, oldFood, newFood, oldFoodSaturation, newFoodSaturation); err != nil {
			return err
		}
	}
