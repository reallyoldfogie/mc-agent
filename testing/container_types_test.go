...
func TestFurnaceInteraction(t *testing.T) {
	...
	serverCfg.CacheDir = filepath.Join(cwd, ".server_cache", "TestFurnaceInteraction", "1.21.5")
...
func TestHopperInteraction(t *testing.T) {
	...
	serverCfg.PullImage = false
	serverCfg.CacheDir = filepath.Join(cwd, ".server_cache", "TestHopperInteraction", "1.21.5")
	RequireIntegrationEnv(t, serverCfg)
