...
// TestFullPipelineLogsToWoodenPickaxe. Unlike every other CraftItem live
// test (agent/craft_test.go, testing/craft_test.go,
...
// TestFullPipelineLogsToWoodenPickaxe exercises the entire
...
// table's 3x3 grid - the scenario docs/plans/CRAFTING_TABLE_3X3_PLAN.md was
// written for.
//
// Unlike ContainerButtonFlatSuite/craft_table_test.go's
// TestCraftItem_TableRequired_WoodenPickaxe, which pre-places the table via
// RCON/setupStandaloneTest to isolate the table-crafting path in a smaller
// test, this test's table only exists because the agent placed it - the
// first live test of agent-driven block placement in this codebase
// (UseItemOnBlock's only other real caller is agent/clutch.go's real-time
// obstacle placement while falling, which has no live test of its own to
// compare against, so this is also the first live confirmation that
...
func (s *CraftPipelineFlatSuite) TestFullPipelineLogsToWoodenPickaxe() {
	...
	leader, err := s.SpawnWorkingAreaAgent("CraftPipelineBot", "craft_pipeline")
	require.NoError(t, err, "spawn agent")

	// 15 minutes, not 90s - see craft_table_test.go's matching comment: the
	// wooden_pickaxe craft at the end of this pipeline also goes through
	// CraftItem's table path, which pathfinds via HPA* and needs the same
	// generous budget this codebase's own navigation live tests use.
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Minute)
	defer cancel()

	// 3 logs -> 12 planks (4 per craft): 4 consumed by the table, 2 by one
	// stick craft (yields 4 sticks, only 2 needed for the pickaxe), 3 by the
	// pickaxe's own top row - 9 consumed total, 3 planks and 2 sticks left
	// over.
	_, err = s.Inst.RCON.Exec(s.Ctx, "give "+leader.Name+" minecraft:oak_log 3")
	require.NoError(t, err, "give oak logs")
	hasLogs, err := waitForAgentHasItem(ctx, leader.Agent, "minecraft:oak_log", 5*time.Second)
	require.NoError(t, err, "wait for logs to sync to agent")
	require.True(t, hasLogs, "agent should see the given oak logs")

	for i := 0; i < 3; i++ {
		require.NoErrorf(t, leader.Agent.CraftItem(ctx, "minecraft:oak_planks"), "craft oak planks (%d/3)", i+1)
	...
	require.NoError(t, leader.Agent.CraftItem(ctx, "minecraft:stick"), "craft sticks from planks")
	time.Sleep(craftSettleDelay)
	...
	require.NoError(t, leader.Agent.CraftItem(ctx, "minecraft:crafting_table"), "craft a crafting table from planks")
	time.Sleep(craftSettleDelay)
	hasTable, err := waitForAgentHasItem(ctx, leader.Agent, "minecraft:crafting_table", 5*time.Second)
	...
	// Place the table: equip it, then right-click the top face of a ground
	// block a couple of blocks away from where the agent is standing - not
	// directly underfoot, which the server rejects as colliding with the
	// agent's own hitbox.
	require.NoError(t, leader.Agent.Equip(ctx, "minecraft:crafting_table"), "equip crafting table")
	...
	require.NoError(t, leader.Agent.UseItemOnBlock(ctx, groundX, groundY, groundZ, models.FaceUp, models.MainHand), "place crafting table")

	_, _, _, placed, err := waitForVisibleCraftingTable(ctx, leader.Agent, 8, 5*time.Second)
	...
	// column) doesn't fit the 2x2 grid, so this exercises CraftItem's table
	// path end to end: it has to re-find the table the agent just placed,
	// walk to it, open it, and craft against its 3x3 grid.
	require.NoError(t, leader.Agent.CraftItem(ctx, "minecraft:wooden_pickaxe"), "craft wooden pickaxe using the placed table")
