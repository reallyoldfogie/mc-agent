...
func TestFindContainersNearby(t *testing.T) {
	...
	serverCfg.CacheDir = filepath.Join(cwd, ".server_cache", "TestFindContainersNearby", "1.21.5")
...
func TestFindAndOpenContainer(t *testing.T) {
	...
	serverCfg.PullImage = false
	serverCfg.CacheDir = filepath.Join(cwd, ".server_cache", "TestFindAndOpenContainer", "1.21.5")
	...
	agentCfg := AgentConfig{
		...
		Version:       serverCfg.Version,
		EnableReplay:  true,
		ReplayOutput:  fmt.Sprintf("./replays/find_open_test_%s.mcpr", time.Now().Format("20060102_150405")),
	}
