...
// SaddleGatingSuite is a version-parameterized suite: one shared flat-world server per
// version instead of one server per test function. Each method spawns its own
// uniquely-named agent in its own working area.
type SaddleGatingSuite struct {
	testingpkg.VersionWorldSuite
}

func TestSaddleGatingSuite(t *testing.T) {
...
func (s *SaddleGatingSuite) TestUnsaddledHorseIsNotDriven() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("NoSaddleBot", "unsaddled_horse_is_not_driven")
...
func (s *SaddleGatingSuite) TestSaddledHorseIsStillDriven() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("SaddledBot", "saddled_horse_is_still_driven")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
