...
func (s *StriderSuite) TestStriderMounting() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("StriderMountBot", "strider_mounting")
...
func (s *StriderSuite) TestStriderMovementOnLand() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("StriderLandBot", "strider_movement_on_land")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *StriderSuite) TestStriderMovementOnLava() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("StriderLavaBot", "strider_movement_on_lava")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx

	// Build a large lava pool (21x21 blocks, radius 10)
	lavaCenterX, lavaCenterY, lavaCenterZ := int(helper.AtX(0)), 1, int(helper.AtZ(0))
	...
	_, err = helper.Instance.RCON.Exec(ctx, fmt.Sprintf("teleport %s %d %d %d", helper.AgentName, lavaCenterX, lavaCenterY+1, lavaCenterZ))
	require.NoError(t, err)
...
func (s *StriderSuite) TestStriderLavaVsLandSpeed() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("StriderSpdBot", "strider_lava_vs_land_speed")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
	...
	lavaCenterX, lavaCenterZ := int(helper.AtX(0)), int(helper.AtZ(0))
	teleportLavaCmd := fmt.Sprintf("teleport %s %d 1 %d", helper.AgentName, lavaCenterX, lavaCenterZ)
...
func (s *StriderSuite) TestStriderSpeedAttribute() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("StrdrSpeedBot", "strider_speed_attribute")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *StriderSuite) TestStriderWarmBlockColdState() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("StriderWarmBot", "strider_warm_block_cold_state")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
	...
	// Keep the recording companion nearby, as the original test did.
	_, err = helper.Instance.RCON.Exec(ctx, fmt.Sprintf("teleport %s %.1f 1 %.1f", leader.Cam.Name, leader.Origin.X+10, leader.Origin.Z+10))
	require.NoError(t, err)
	time.Sleep(500 * time.Millisecond)

	_, err = helper.Instance.RCON.Exec(ctx, fmt.Sprintf("effect give %s fire_resistance infinite 0 false", helper.AgentName))
	require.NoError(t, err)
	time.Sleep(500 * time.Millisecond)
	...
	killResponse, err := helper.Instance.RCON.Exec(ctx, fmt.Sprintf("kill @e[type=minecraft:strider,x=%.1f,y=%.1f,z=%.1f,distance=..40]", x, y, z))
...
func (s *StriderSuite) TestStriderControllability() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("StrdrCtrlBot", "strider_controllability")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
	...
	// ── Test WITH fungus ──
	_, err = helper.Instance.RCON.Exec(ctx, fmt.Sprintf("teleport %s %d 1 %d", helper.AgentName, int64(startPos.X), int64(startPos.Z)))
	require.NoError(t, err)
	time.Sleep(500 * time.Millisecond)
