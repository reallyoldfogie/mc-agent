...
func (s *HappyGhastSuite) TestHappyGhastMounting() {
	t := s.T()
	...
	leader, spawnErr := s.SpawnWorkingAreaAgent("GhastMountBot", "happy_ghast_mounting")
...
func (s *HappyGhastSuite) TestUnharnessedHappyGhastIsNotDriven() {
	t := s.T()
	if !isVersionGreaterOrEqual(s.Version, happyGhastMinVersion) {
		t.Skipf("Happy ghast not available in version %s (requires %s+)", s.Version, happyGhastMinVersion)
	}

	leader, spawnErr := s.SpawnWorkingAreaAgent("NoHarnessBot", "unharnessed_happy_ghast_is_not_driven")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *HappyGhastSuite) TestHarnessedHappyGhastIsStillDriven() {
	t := s.T()
	if !isVersionGreaterOrEqual(s.Version, happyGhastMinVersion) {
		t.Skipf("Happy ghast not available in version %s (requires %s+)", s.Version, happyGhastMinVersion)
	}

	leader, spawnErr := s.SpawnWorkingAreaAgent("HarnessBot", "harnessed_happy_ghast_is_still_driven")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *HappyGhastSuite) TestHappyGhastHoversWithZeroGravity() {
	t := s.T()
	if !isVersionGreaterOrEqual(s.Version, happyGhastMinVersion) {
		t.Skipf("Happy ghast not available in version %s (requires %s+)", s.Version, happyGhastMinVersion)
	}

	leader, spawnErr := s.SpawnWorkingAreaAgent("GhastHoverBot", "happy_ghast_hovers_with_zero_gravity")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *HappyGhastSuite) TestHappyGhastFlightMovement() {
	t := s.T()
	if !isVersionGreaterOrEqual(s.Version, happyGhastMinVersion) {
		t.Skipf("Happy ghast not available in version %s (requires %s+)", s.Version, happyGhastMinVersion)
	}

	leader, spawnErr := s.SpawnWorkingAreaAgent("GhastFlyBot", "happy_ghast_flight_movement")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
