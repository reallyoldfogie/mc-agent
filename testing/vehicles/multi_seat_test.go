...
func (s *MultiSeatSuite) TestMultiSeatBoatDriverPromotion() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("SeatDriver", "multi_seat_boat_driver_promotion")
	...
	_, err = helper.Instance.RCON.Exec(ctx, fmt.Sprintf("teleport %s %.1f 0 %.1f", rider.Name, helper.AtX(-1), helper.AtZ(2)))
...
func (s *MultiSeatSuite) TestMultiSeatHappyGhastPromotion() {
	t := s.T()
	...
	leader, spawnErr := s.SpawnWorkingAreaAgent("GhastSeat0", "multi_seat_happy_ghast_promotion")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
