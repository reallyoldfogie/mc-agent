...
func (s *EntityAttributesSuite) TestEntityAttributeTracking() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("EntityAttrBot", "entity_attribute_tracking")
...
func (s *EntityAttributesSuite) TestHorseAttributesAfterMounting() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("HrsAttrAftrMntBt", "horse_attributes_after_mounting")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *EntityAttributesSuite) TestMultipleEntityAttributes() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("MltplEntyAttrBot", "multiple_entity_attributes")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
