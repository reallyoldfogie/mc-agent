...
// PerceptionSuite is a version-parameterized suite: one shared flat-world server per
...
func (s *PerceptionSuite) TestFindRideableEntitiesNearHonorsBlindness() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("PerceptionBot", "find_rideable_entities_near_honors_blindness")
