...
func (s *CamelSuite) TestCamelMounting() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelMountBot", "camel_mounting")
...
func (s *CamelSuite) TestCamelMovement() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelMoveBot", "camel_movement")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *CamelSuite) TestCamelSpeedAttribute() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelSpdBot", "camel_speed_attribute")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *CamelSuite) TestCamelLunging() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelJumpBot", "camel_lunging")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *CamelSuite) TestCamelWaterBehavior() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelWaterBot", "camel_water_behavior")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx

	// Determine expected behavior based on version
	isNewBehavior := isVersionGreaterOrEqual(s.Version, "1.21.11")
	behaviorDesc := "float (1.21.11+)"
	if !isNewBehavior {
		behaviorDesc = "sink (pre-1.21.11)"
	}
	t.Logf("Testing camel water behavior for %s: expected %s", s.Version, behaviorDesc)
...
func (s *CamelSuite) TestCamelSittingDetectionUnmounted() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelDetBot", "camel_sitting_detection_unmounted")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *CamelSuite) TestCamelSittingDetection() {
	t := s.T()
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelSitBot", "camel_sitting_detection")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
func (s *CamelSuite) TestCamelHuskMounting() {
	t := s.T()
	// Camel husk entity was added in 1.21.11
	if !isVersionGreaterOrEqual(s.Version, "1.21.11") {
		t.Skipf("Camel husk entity not available in version %s (requires 1.21.11+)", s.Version)
	}

	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelHuskMountBot", "camel_husk_mounting")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
// Camel husk was added in Minecraft 1.21.11.
func (s *CamelSuite) TestCamelHuskMovement() {
	t := s.T()
	// Camel husk entity was added in 1.21.11
	if !isVersionGreaterOrEqual(s.Version, "1.21.11") {
		t.Skipf("Camel husk entity not available in version %s (requires 1.21.11+)", s.Version)
	...
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelHuskMoveBot", "camel_husk_movement")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx
...
// Camel husk was added in Minecraft 1.21.11.
func (s *CamelSuite) TestCamelHuskDashCharging() {
	t := s.T()
	// Camel husk entity was added in 1.21.11
	if !isVersionGreaterOrEqual(s.Version, "1.21.11") {
		t.Skipf("Camel husk entity not available in version %s (requires 1.21.11+)", s.Version)
	...
	leader, spawnErr := s.SpawnWorkingAreaAgent("CamelHuskDashBot", "camel_husk_dash_charging")
	require.NoError(t, spawnErr, "spawn agent")
	helper := NewVehicleTestHelperForSuite(&s.VersionWorldSuite, leader)
	ctx := s.Ctx

	// Create a large arena for dash testing
	_, err := helper.Instance.RCON.Exec(ctx, fmt.Sprintf("fill %.0f -1 %.0f %.0f 5 %.0f stone", helper.AtX(0), helper.AtZ(0), helper.AtX(50), helper.AtZ(50)))
	require.NoError(t, err)
