...
// componentTypeNameToID/componentTypeIDToName translate between 1.21.4's own
...
func (c *containerHandler) SendUseItemOn(conn models.PacketWriter, hand models.Hand, x, y, z int, face int32, cursorX, cursorY, cursorZ float32, insideBlock bool, sequence int32) error {
	...
	pkt.WorldBorderHit = pk.Boolean(false) // 1.21.4+ has this field
...
// In 1.21.4, the slot field is varint (int32).
...
// DecodeSlot reads one full (non-hashed) Slot in 1.21.4's real wire format --
...
func (c *containerHandler) DecodeSlot(r io.Reader) (screen.Slot, int64, error) {
	...
	if !ok {
		return screen.Slot{}, n, fmt.Errorf("v1_21_4: unexpected non-empty slot payload type %T", wireSlot.UnnamedType0001)
	}
	...
	if components := def.Components.Get(); components != nil {
		for _, component := range *components {
			...
			if !ok {
				return screen.Slot{}, n, fmt.Errorf("v1_21_4: unknown slot component type %q", component.Type.Value)
			}
	...
	if removeComponents := def.RemoveComponents.Get(); removeComponents != nil {
		for _, rc := range *removeComponents {
			...
			if !ok {
				return screen.Slot{}, n, fmt.Errorf("v1_21_4: unknown slot component type %q (remove)", rc.Type.Value)
			}
...
// slotFromScreenSlot converts mc-bot-go's screen.Slot into 1.21.4's own
// plain-Slot wire format, encoding every component with its real payload
// (data/1.21.4/basetypes.SlotComponent's generated per-type WriteTo) rather
...
func slotFromScreenSlot(slot *screen.Slot) (basetypes.Slot, error) {
	...
	for _, component := range slot.Components {
		...
		if !ok {
			return basetypes.Slot{}, fmt.Errorf("v1_21_4: unknown slot component type ID %d", component.Type)
		}
		field, ok := component.Data.(pk.Field)
		if !ok {
			return basetypes.Slot{}, fmt.Errorf("v1_21_4: slot component data does not implement pk.Field: %T", component.Data)
		}
	...
	for _, componentType := range slot.RemoveComponents {
		...
		if !ok {
			return basetypes.Slot{}, fmt.Errorf("v1_21_4: unknown slot component type ID %d (remove)", componentType)
		}
...
// packet in 1.21.4's real wire format, using mc-bot-go's Slot type directly
...
func (c *containerHandler) SendContainerButtonClick(conn models.PacketWriter, windowID int8, buttonID int8) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.4 Container] SendContainerButtonClick: windowID=%d buttonID=%d", windowID, buttonID))
