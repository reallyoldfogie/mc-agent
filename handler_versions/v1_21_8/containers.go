...
// componentTypeNameToID/componentTypeIDToName translate between 1.21.8's own
...
// containerHandler implements common.ContainerHandler for 1.21.8.
...
// SendPickItem
// Note: In 1.21.8, the PickItem packet takes a block position, not a slot.
...
func (c *containerHandler) SendPickItem(conn models.PacketWriter, slot int32) error {
	// The 1.21.8 protocol changed PickItem to PickItemFromBlock which takes a position
	// instead of a slot. This interface needs to be redesigned for 1.21.8+.
...
// Note: 1.21.8+ includes the WorldBorderHit field (added in 1.21.2/protocol 768).
func (c *containerHandler) SendUseItemOn(conn models.PacketWriter, hand models.Hand, x, y, z int, face int32, cursorX, cursorY, cursorZ float32, insideBlock bool, sequence int32) error {
	...
	pkt.WorldBorderHit = pk.Boolean(false) // 1.21.8+ has this field
...
func convertSlotFromProtocol(slot basetypes.Slot) models.InventorySlot {
	...
	// The slot structure in 1.21.8 uses ItemCount > 0 to indicate presence
...
// In 1.21.8, the slot field is varint (int32).
func (c *containerHandler) ParseHeldItemSlot(p pk.Packet) (int16, error) {
	...
	// In 1.21.8, Slot is pk.VarInt (int32)
	return int16(pkt.Slot), nil
}

// DecodeSlot reads one full (non-hashed) Slot in 1.21.8's real wire format --
...
func (c *containerHandler) DecodeSlot(r io.Reader) (screen.Slot, int64, error) {
	...
	if !ok {
		return screen.Slot{}, n, fmt.Errorf("v1_21_8: unexpected non-empty slot payload type %T", wireSlot.UnnamedType0001)
	}
	...
	if components := def.Components.Get(); components != nil {
		for _, component := range *components {
			...
			if !ok {
				return screen.Slot{}, n, fmt.Errorf("v1_21_8: unknown slot component type %q", component.Type.Value)
			}
	...
	if removeComponents := def.RemoveComponents.Get(); removeComponents != nil {
		for _, rc := range *removeComponents {
			...
			if !ok {
				return screen.Slot{}, n, fmt.Errorf("v1_21_8: unknown slot component type %q (remove)", rc.Type.Value)
			}
...
// hashedSlotFromScreenSlot converts mc-bot-go's screen.Slot into 1.21.8's own
...
func hashedSlotFromScreenSlot(slot *screen.Slot) (protocol_models.Option[basetypes.HashedSlot], error) {
	...
	for _, component := range slot.Components {
		...
		if !ok {
			return protocol_models.Option[basetypes.HashedSlot]{}, fmt.Errorf("v1_21_8: unknown slot component type ID %d", component.Type)
		}
	...
	for _, componentType := range slot.RemoveComponents {
		...
		if !ok {
			return protocol_models.Option[basetypes.HashedSlot]{}, fmt.Errorf("v1_21_8: unknown slot component type ID %d (remove)", componentType)
		}
...
// packet in 1.21.8's real wire format (HashedSlot item encoding, VarInt
...
func (c *containerHandler) SendContainerButtonClick(conn models.PacketWriter, windowID int8, buttonID int8) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.8 Container] SendContainerButtonClick: windowID=%d buttonID=%d", windowID, buttonID))
