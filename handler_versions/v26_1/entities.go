...
// entityHandler implements common.EntityHandler for 26.1.
...
func (e *entityHandler) ParseAddEntity(p pk.Packet) (entityID, entityType, objectData int32, uuid [16]byte, x, y, z float64, yaw, pitch int8, velX, velY, velZ float64, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseAddEntity] returning %d %d %d %v %.2f %.2f %.2f %d %d vel=(%.4f,%.4f,%.4f) <nil>", entityID, entityType, objectData, uuid, x, y, z, yaw, pitch, velX, velY, velZ))
...
func (e *entityHandler) ParseMoveEntityPos(p pk.Packet) (entityID int32, dx, dy, dz int16, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseMoveEntityPos] %d received %s", entityID, spew.Sdump(pkt)))
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseMoveEntityPos] returning %d %d %d %d %v <nil>", entityID, dx, dy, dz, onGround))
...
func (e *entityHandler) ParseMoveEntityPosRot(p pk.Packet) (entityID int32, dx, dy, dz int16, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseMoveEntityPosRot] returning %d %d %d %d %d %d %v <nil>", entityID, dx, dy, dz, yaw, pitch, onGround))
...
func (e *entityHandler) ParseTeleportEntity(p pk.Packet) (entityID int32, x, y, z float64, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseTeleportEntity] returning %d %.2f %.2f %.2f %d %d %v <nil>", entityID, x, y, z, yaw, pitch, onGround))
...
func (e *entityHandler) ParseSyncEntityPosition(p pk.Packet) (entityID int32, x, y, z float64, dx, dy, dz float64, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseSyncEntityPosition] returning %d pos=(%.2f,%.2f,%.2f) vel=(%.4f,%.4f,%.4f) rot=(%d,%d) ground=%v <nil>",
...
func (e *entityHandler) ParseRemoveEntities(p pk.Packet) (entityIDs []int32, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseRemoveEntities] returning %v <nil>", entityIDs))
...
func (e *entityHandler) ParseEntityEvent(p pk.Packet) (entityID int32, eventID int8, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseEntityEvent] returning %d %d <nil>", entityID, eventID))
...
func (e *entityHandler) ParseSetEntityMetadata(p pk.Packet) (entityID int32, entries []models.MetadataEntry, err error) {
	...
	for i, entry := range pkt.Metadata.Entries {
		...
		utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseSetEntityMetadata] Entry %d: key=%d handler=%s value=%T",
			i, entry.Key, handlerID.String(), entry.Value))
	}

	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseSetEntityMetadata] returning entityID=%d with %d metadata entries <nil>", entityID, len(entries)))
...
func (e *entityHandler) ParseEntityVelocityUpdate(p pk.Packet) (entityID int32, velX, velY, velZ float64, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseEntityVelocityUpdate] Raw parsed packet: %#v", *pkt))
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseEntityVelocityUpdate] returning %d %.6f %.6f %.6f <nil>", entityID, velX, velY, velZ))
...
func (e *entityHandler) ParseEntityEquipment(p pk.Packet) (entityID int32, equipment []models.EquipmentEntry, err error) {
	...
	for _, entry := range pkt.Equipments.Values {
		...
		utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v26.1 Entity][ParseEntityEquipment]: entityID=%d, slot=%d, itemCount=%d",
...
func (e *entityHandler) ParseEntityHeadRotation(p pk.Packet) (entityID int32, headYaw int8, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseEntityHeadRotation] returning %d %d <nil>", entityID, headYaw))
...
func (e *entityHandler) ParseEntityLook(p pk.Packet) (entityID int32, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[26.1][ParseEntityLook] returning %d %d %d %v <nil>", entityID, yaw, pitch, onGround))

	return entityID, yaw, pitch, onGround, nil
}

// handToUseEntityHand maps models.Hand to the 26.1 UseEntity mapper enum value.
// 26.1's generated UseEntityHand is a string-based mapper ("main_hand"/"off_hand"),
// matching UseEntityHandMappings in mc-protocol-go/data/26.1/play/serverbound/packet_mapper.go.
func handToUseEntityHand(hand models.Hand) sb.UseEntityHand {
	if hand == models.OffHand {
		return sb.UseEntityHand{Value: "off_hand"}
	}
	return sb.UseEntityHand{Value: "main_hand"}
}

// SendInteract sends an entity interaction packet (right-click with hand).
//
// Protocol note: 26.1 restructured UseEntity - it dropped the "mouse" switch
// field (which previously distinguished Interact/Attack/InteractAt via 0/1/2)
// and now unconditionally carries an LpVec3 Location. Attack was split out
// into its own standalone Attack packet (see SendAttack below). For a plain
// interact (no target position), Location is sent as the zero vector.
func (e *entityHandler) SendInteract(conn models.PacketWriter, entityID int32, hand models.Hand, sneaking bool) error {
	...
	pkt.Hand = handToUseEntityHand(hand)
	pkt.Location = protocol_models.LpVec3{X: 0, Y: 0, Z: 0}
	pkt.Sneaking = pk.Boolean(sneaking)

	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v26.1 Entity] SendInteract: entityID=%d hand=%d sneaking=%v", entityID, hand, sneaking))
...
func (e *entityHandler) SendInteractAt(conn models.PacketWriter, entityID int32, targetX, targetY, targetZ float32, hand models.Hand, sneaking bool) error {
	...
	pkt.Target = pk.VarInt(entityID)
	pkt.Hand = handToUseEntityHand(hand)
	pkt.Location = protocol_models.LpVec3{X: float64(targetX), Y: float64(targetY), Z: float64(targetZ)}
	pkt.Sneaking = pk.Boolean(sneaking)

	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v26.1 Entity] SendInteractAt: entityID=%d pos=(%.2f,%.2f,%.2f) hand=%d sneaking=%v",
...
//
// Protocol note: 26.1 split Attack out of UseEntity into its own standalone
// packet carrying only the target entity ID - there is no longer a Hand,
// Location, or Sneaking field on this packet (sneaking state is already
// tracked server-side via the PlayerInput packet's Shift flag).
func (e *entityHandler) SendAttack(conn models.PacketWriter, entityID int32, sneaking bool) error {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v26.1 Entity] SendAttack: entityID=%d sneaking=%v", entityID, sneaking))

	if err := conn.WritePacket(pkt.Marshal()); err != nil {
		return common.ErrPacketSend{PacketName: "Attack", Cause: err}
	}
	return nil
}
...
func (e *entityHandler) ParseDamageEvent(p pk.Packet) (entityID int32, sourceTypeID int32, sourceCauseID int32, sourceDirectID int32, sourceX, sourceY, sourceZ float64, hasSourcePosition bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[%s][ParseDamageEvent] entityID=%d sourceType=%d causedBy=%d directBy=%d hasPos=%v pos=(%.2f,%.2f,%.2f)",
		"26.1", entityID, sourceTypeID, sourceCauseID, sourceDirectID, hasSourcePosition, sourceX, sourceY, sourceZ))
...
func (e *entityHandler) ParseSetPassengers(p pk.Packet) (vehicleEntityID int32, passengerEntityIDs []int32, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v26.1 Entity] ParseSetPassengers: vehicleID=%d, passengers=%v", vehicleEntityID, passengerEntityIDs))
...
// net/minecraft/world/entity/ai/attributes/Attributes.java's registration
// order (this version renamed/moved the registry class from 1.21.11's
// net.minecraft.entity.attribute.EntityAttributes, but the registration
// order and the resulting gap are unchanged — same "Group C" bug). See
// mc-protocol-go/docs/bugs/entity-attribute-mapper-missing-attributes.md
// for the full analysis with citations, tracked for an eventual upstream
// fix via a PrismarineJS/minecraft-data patch. TEMPORARY — delete this
// function and its call site once that lands.
//
