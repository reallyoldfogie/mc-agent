...
import (
	...
	cb "github.com/reallyoldfogie/mc-protocol-go/data/1.21.9/login/clientbound"
	sb "github.com/reallyoldfogie/mc-protocol-go/data/1.21.9/login/serverbound"
	protocol_models "github.com/reallyoldfogie/mc-protocol-go/models"
)

// loginHandler implements models.LoginHandler for 1.21.9.
...
func (l *loginHandler) SendLoginStart(conn models.PacketWriter, username string, uuid [16]byte) error {
	...
	utils.SafeLogger(l.logger).Debug(fmt.Sprintf("[v1.21.9 Login] SendLoginStart: username=%s uuid=%x", username, uuid))
...
func (l *loginHandler) SendEncryptionResponse(conn models.PacketWriter, sharedSecret, verifyToken []byte) error {
	...
	utils.SafeLogger(l.logger).Debug(fmt.Sprintf("[v1.21.9 Login] SendEncryptionResponse: sharedSecretLen=%d verifyTokenLen=%d",
...
func (l *loginHandler) SendLoginAcknowledged(conn models.PacketWriter) error {
	...
	utils.SafeLogger(l.logger).Debug(fmt.Sprintf("[v1.21.9 Login] SendLoginAcknowledged"))
...
func (l *loginHandler) ParseLoginSuccess(p pk.Packet) (username string, uuid [16]byte, err error) {
	...
	utils.SafeLogger(l.logger).Debug(fmt.Sprintf("[v1.21.9 Login] ParseLoginSuccess: username=%s uuid=%x", username, uuid))
...
func (l *loginHandler) ParseEncryptionRequest(p pk.Packet) (serverID string, publicKey, verifyToken []byte, err error) {
	...
	utils.SafeLogger(l.logger).Debug(fmt.Sprintf("[v1.21.9 Login] ParseEncryptionRequest: serverID=%s publicKeyLen=%d verifyTokenLen=%d",
