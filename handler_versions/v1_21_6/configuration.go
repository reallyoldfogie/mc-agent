...
import (
	...
	cb "github.com/reallyoldfogie/mc-protocol-go/data/1.21.6/configuration/clientbound"
	sb "github.com/reallyoldfogie/mc-protocol-go/data/1.21.6/configuration/serverbound"
	protocol_models "github.com/reallyoldfogie/mc-protocol-go/models"
)

// configurationHandler implements common.ConfigurationHandler for 1.21.6.
...
func (c *configurationHandler) SendFinishConfiguration(conn models.PacketWriter) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] SendFinishConfiguration"))
...
func (c *configurationHandler) SendKeepAlive(conn models.PacketWriter, id int64) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] SendKeepAlive: id=%d", id))
...
func (c *configurationHandler) SendPong(conn models.PacketWriter, pingID int32) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] SendPong: pingID=%d", pingID))
...
// Note: In 1.21.6, client information is typically sent via a custom payload during configuration.
func (c *configurationHandler) SendClientInformation(conn models.PacketWriter, info models.ClientInfo) error {
	...
	// the specific protocol requirements for 1.21.6
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] SendClientInformation: locale=%s viewDistance=%d", info.Locale, info.ViewDistance))
...
func (c *configurationHandler) SendResourcePackResponse(conn models.PacketWriter, uuid [16]byte, result int32) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] SendResourcePackResponse: uuid=%x result=%d", uuid, result))
...
func (c *configurationHandler) ParseRegistryData(p pk.Packet) (registryID string, entries map[string]int32, err error) {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] ParseRegistryData: registryID=%s entriesCount=%d", registryID, len(entries)))
...
func (c *configurationHandler) ParseKeepAlive(p pk.Packet) (id int64, err error) {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] ParseKeepAlive: id=%d", id))
...
func (c *configurationHandler) ParsePing(p pk.Packet) (pingID int32, err error) {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] ParsePing: pingID=%d", pingID))
...
func (c *configurationHandler) ParseDisconnect(p pk.Packet) (reason string, err error) {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.6 Configuration] ParseDisconnect: reason=%s", reason))
