...
// componentTypeNameToID/componentTypeIDToName translate between 1.21.3's own
...
// containerHandler implements common.ContainerHandler for 1.21.3.
...
func (c *containerHandler) SendContainerClick(conn models.PacketWriter, windowID int8, stateID, slot int32, button int8, mode int32, changedSlots map[int16]models.InventorySlot, carriedItem models.InventorySlot) error {
	...
	// Note: In 1.21.3, Slot structure uses ItemCount + UnnamedType0003 switch field
	...
	for slotNum, slotData := range changedSlots {
		...
		// Set the item data - in 1.21.3, Slot uses ItemCount and UnnamedType0003
		if slotData.Present {
...
// SendPickItem
// Note: In 1.21.3, the PickItem packet takes a block position, not a slot.
...
func (c *containerHandler) SendPickItem(conn models.PacketWriter, slot int32) error {
	// The 1.21.3 protocol changed PickItem to PickItemFromBlock which takes a position
	// instead of a slot. This interface needs to be redesigned for 1.21.3+.
...
// Note: 1.21.3+ includes the WorldBorderHit field (added in 1.21.3/protocol 768).
func (c *containerHandler) SendUseItemOn(conn models.PacketWriter, hand models.Hand, x, y, z int, face int32, cursorX, cursorY, cursorZ float32, insideBlock bool, sequence int32) error {
	...
	pkt.WorldBorderHit = pk.Boolean(false) // 1.21.3+ has this field
...
// In 1.21.3, the slot field is i8 (byte).
func (c *containerHandler) ParseHeldItemSlot(p pk.Packet) (int16, error) {
	...
	// In 1.21.3, Slot is pk.Byte (int8)
	return int16(pkt.Slot), nil
}

// DecodeSlot reads one full (non-hashed) Slot in 1.21.3's real wire format --
...
func (c *containerHandler) DecodeSlot(r io.Reader) (screen.Slot, int64, error) {
	...
	if !ok {
		return screen.Slot{}, n, fmt.Errorf("v1_21_3: unexpected non-empty slot payload type %T", wireSlot.UnnamedType0001)
	}
	...
	if components := def.Components.Get(); components != nil {
		for _, component := range *components {
			...
			if !ok {
				return screen.Slot{}, n, fmt.Errorf("v1_21_3: unknown slot component type %q", component.Type.Value)
			}
	...
	if removeComponents := def.RemoveComponents.Get(); removeComponents != nil {
		for _, rc := range *removeComponents {
			...
			if !ok {
				return screen.Slot{}, n, fmt.Errorf("v1_21_3: unknown slot component type %q (remove)", rc.Type.Value)
			}
...
// slotFromScreenSlot converts mc-bot-go's screen.Slot into 1.21.3's own
// plain-Slot wire format, encoding every component with its real payload
// (data/1.21.3/basetypes.SlotComponent's generated per-type WriteTo) rather
...
func slotFromScreenSlot(slot *screen.Slot) (basetypes.Slot, error) {
	...
	for _, component := range slot.Components {
		...
		if !ok {
			return basetypes.Slot{}, fmt.Errorf("v1_21_3: unknown slot component type ID %d", component.Type)
		}
		field, ok := component.Data.(pk.Field)
		if !ok {
			return basetypes.Slot{}, fmt.Errorf("v1_21_3: slot component data does not implement pk.Field: %T", component.Data)
		}
	...
	for _, componentType := range slot.RemoveComponents {
		...
		if !ok {
			return basetypes.Slot{}, fmt.Errorf("v1_21_3: unknown slot component type ID %d (remove)", componentType)
		}
...
// packet in 1.21.3's real wire format, using mc-bot-go's Slot type directly
...
func (c *containerHandler) SendContainerButtonClick(conn models.PacketWriter, windowID int8, buttonID int8) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.3 Container] SendContainerButtonClick: windowID=%d buttonID=%d", windowID, buttonID))
