...
// ComponentHash computes the CRC32C "hashed" representation of a single
// decoded item component's Data value, matching vanilla's
// HashedStack/CrcHashOps algorithm used for ServerboundContainerClick's
// item slots from protocol 1.21.5 onward. Every per-version
// containers.go's SendContainerClickV2 (for versions using HashedSlot)
// calls this while building each basetypes.HashedSlotComponentsArrayType
// entry -- ported once here from mc-bot-go's bot/screen/hashops.go
// (unexported there) instead of duplicating this ~250 line algorithm in
// every version file, since the hashing itself has no per-version
// variation (only the component name<->ID numbering, handled separately
// per version, does).
...
// BuildSlotComponentTypeMaps builds the name<->numeric-ID lookup tables a
// version's DecodeSlot/SendContainerClickV2 use to translate between
// mc-bot-go's screen.SlotComponent.Type (a numeric ID, arbitrary but
// consistent within one version's own codec) and this version's own
// generated basetypes.SlotComponentTypeMappings (keyed by the real
// protocol numeric ID for that exact version). Call once at package init
// with that version's own mappings -- mirrors mc-bot-go hashops.go's
// buildComponentTypeNameToID/buildComponentTypeIDToName, but per real
// version instead of per pre/post-1.21.5 bucket.
func BuildSlotComponentTypeMaps(mappings map[int64]string) (nameToID map[string]int32, idToName map[int32]string) {
	nameToID = make(map[string]int32, len(mappings))
	idToName = make(map[int32]string, len(mappings))
	for id, name := range mappings {
		nameToID[name] = int32(id)
		idToName[int32(id)] = name
	}
