...
func (e *entityHandler) ParseAddEntity(p pk.Packet) (entityID, entityType, objectData int32, uuid [16]byte, x, y, z float64, yaw, pitch int8, velX, velY, velZ float64, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseAddEntity] returning %d %d %d %v %.2f %.2f %.2f %d %d vel=(%.4f,%.4f,%.4f) <nil>", entityID, entityType, objectData, uuid, x, y, z, yaw, pitch, velX, velY, velZ))
...
func (e *entityHandler) ParseMoveEntityPos(p pk.Packet) (entityID int32, dx, dy, dz int16, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseMoveEntityPos] %d received %s", entityID, spew.Sdump(pkt)))
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseMoveEntityPos] returning %d %d %d %d %v <nil>", entityID, dx, dy, dz, onGround))
...
func (e *entityHandler) ParseMoveEntityPosRot(p pk.Packet) (entityID int32, dx, dy, dz int16, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseMoveEntityPosRot] returning %d %d %d %d %d %d %v <nil>", entityID, dx, dy, dz, yaw, pitch, onGround))
...
func (e *entityHandler) ParseTeleportEntity(p pk.Packet) (entityID int32, x, y, z float64, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseTeleportEntity] returning %d %.2f %.2f %.2f %d %d %v <nil>", entityID, x, y, z, yaw, pitch, onGround))
...
func (e *entityHandler) ParseSyncEntityPosition(p pk.Packet) (entityID int32, x, y, z float64, dx, dy, dz float64, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseSyncEntityPosition] returning %d pos=(%.2f,%.2f,%.2f) vel=(%.4f,%.4f,%.4f) rot=(%d,%d) ground=%v <nil>",
...
func (e *entityHandler) ParseRemoveEntities(p pk.Packet) (entityIDs []int32, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseRemoveEntities] returning %v <nil>", entityIDs))
...
func (e *entityHandler) ParseEntityEvent(p pk.Packet) (entityID int32, eventID int8, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseEntityEvent] returning %d %d <nil>", entityID, eventID))
...
func (e *entityHandler) ParseSetEntityMetadata(p pk.Packet) (entityID int32, entries []models.MetadataEntry, err error) {
	...
	for i, entry := range pkt.Metadata.Entries {
		...
		utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseSetEntityMetadata] Entry %d: key=%d handler=%s value=%T",
			i, entry.Key, handlerID.String(), entry.Value))
	}

	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseSetEntityMetadata] returning entityID=%d with %d metadata entries <nil>", entityID, len(entries)))
...
func (e *entityHandler) ParseEntityVelocityUpdate(p pk.Packet) (entityID int32, velX, velY, velZ float64, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseEntityVelocityUpdate] Raw parsed packet: %#v", *pkt))
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseEntityVelocityUpdate] returning %d %.6f %.6f %.6f <nil>", entityID, velX, velY, velZ))
...
func (e *entityHandler) ParseEntityEquipment(p pk.Packet) (entityID int32, equipment []models.EquipmentEntry, err error) {
	...
	for _, entry := range pkt.Equipments.Values {
		...
		utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v1.21.11 Entity][ParseEntityEquipment]: entityID=%d, slot=%d, itemCount=%d",
...
func (e *entityHandler) ParseEntityHeadRotation(p pk.Packet) (entityID int32, headYaw int8, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseEntityHeadRotation] returning %d %d <nil>", entityID, headYaw))
...
func (e *entityHandler) ParseEntityLook(p pk.Packet) (entityID int32, yaw, pitch int8, onGround bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[1.21.11][ParseEntityLook] returning %d %d %d %v <nil>", entityID, yaw, pitch, onGround))
...
func (e *entityHandler) SendInteract(conn models.PacketWriter, entityID int32, hand models.Hand, sneaking bool) error {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v1.21.11 Entity] SendInteract: entityID=%d hand=%d sneaking=%v", entityID, hand, sneaking))
...
func (e *entityHandler) SendInteractAt(conn models.PacketWriter, entityID int32, targetX, targetY, targetZ float32, hand models.Hand, sneaking bool) error {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v1.21.11 Entity] SendInteractAt: entityID=%d pos=(%.2f,%.2f,%.2f) hand=%d sneaking=%v",
...
func (e *entityHandler) SendAttack(conn models.PacketWriter, entityID int32, sneaking bool) error {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v1.21.11 Entity] SendAttack: entityID=%d sneaking=%v", entityID, sneaking))
...
func (e *entityHandler) ParseDamageEvent(p pk.Packet) (entityID int32, sourceTypeID int32, sourceCauseID int32, sourceDirectID int32, sourceX, sourceY, sourceZ float64, hasSourcePosition bool, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[%s][ParseDamageEvent] entityID=%d sourceType=%d causedBy=%d directBy=%d hasPos=%v pos=(%.2f,%.2f,%.2f)",
		"1.21.11", entityID, sourceTypeID, sourceCauseID, sourceDirectID, hasSourcePosition, sourceX, sourceY, sourceZ))
...
func (e *entityHandler) ParseSetPassengers(p pk.Packet) (vehicleEntityID int32, passengerEntityIDs []int32, err error) {
	...
	utils.SafeLogger(e.logger).Debug(fmt.Sprintf("[v1.21.11 Entity] ParseSetPassengers: vehicleID=%d, passengers=%v", vehicleEntityID, passengerEntityIDs))
