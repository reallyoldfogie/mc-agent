...
// movementHandler implements common.MovementHandler for 1.21.11.
...
func (m *movementHandler) SendPosition(conn models.PacketWriter, x, y, z float64, onGround bool) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendPosition: (%.2f, %.2f, %.2f) onGround=%v", x, y, z, onGround))
...
func (m *movementHandler) SendPositionAndRotation(conn models.PacketWriter, x, y, z float64, yaw, pitch float64, onGround bool) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendPositionAndRotation[ID=%d (0x%X)]: (%.2f, %.2f, %.2f) yaw=%.2f pitch=%.2f onGround=%v",
...
func (m *movementHandler) SendPlayerCommand(conn models.PacketWriter, entityID, actionID int32) error {
	// Handle sneaking via PlayerInput packet (new in 1.21.11)
	...
	if !ok {
		return common.ErrPacketSend{PacketName: "EntityAction", Cause: fmt.Errorf("unsupported actionID %d in 1.21.11", actionID)}
	}
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendPlayerCommand: entityID=%d actionID=%d -> %s", entityID, actionID, actionStr))
...
// This is the new packet in 1.21.11 for handling movement inputs including sneaking.
func (m *movementHandler) sendPlayerInput(conn models.PacketWriter) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendPlayerInput: shift=%v sprint=%v", m.isSneaking, m.isSprinting))
...
func (m *movementHandler) SendTeleportConfirm(conn models.PacketWriter, teleportID int32) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendTeleportConfirm: teleportID=%d", teleportID))
...
func (m *movementHandler) SendPlayerAbilities(conn models.PacketWriter, flags byte) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendPlayerAbilities: flags=%d", flags))
...
func (m *movementHandler) SendMoveVehicle(conn models.PacketWriter, x, y, z float64, yaw, pitch float64, onGround bool) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendMoveVehicle: (%.2f, %.2f, %.2f) yaw=%.2f pitch=%.2f onGround=%v", x, y, z, yaw, pitch, onGround))
...
func (m *movementHandler) SendVehicleInput(conn models.PacketWriter, forward, backward, left, right, jump, sneak bool) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendVehicleInput: forward=%v backward=%v left=%v right=%v jump=%v sneak=%v",
...
// v1.21.11+ uses string-based action IDs in EntityAction.
func (m *movementHandler) SendPlayerCommandWithParam(conn models.PacketWriter, entityID, actionID, jumpBoost int32) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[v1.21.11 Movement] SendPlayerCommandWithParam: entityID=%d actionID=%d -> %s jumpBoost=%d", entityID, actionID, actionStr, jumpBoost))
...
func (m *movementHandler) SendBoatPaddleState(conn models.PacketWriter, leftPaddling, rightPaddling bool) error {
	...
	utils.DebugVerbose(m.logger, fmt.Sprintf("[%s Movement] SendBoatPaddleState: left=%v right=%v", "%s", leftPaddling, rightPaddling))
