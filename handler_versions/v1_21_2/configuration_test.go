...
import (
	...
	cb "github.com/reallyoldfogie/mc-protocol-go/data/1.21.2/configuration/clientbound"
	sb "github.com/reallyoldfogie/mc-protocol-go/data/1.21.2/configuration/serverbound"
)
...
func TestConfigurationHandler_SendFinishConfiguration_PacketStructure(t *testing.T) {
	...
	// Verify packet ID is correct for 1.21.2 configuration phase
	if pkt.PacketID() != 3 {
...
func TestConfigurationHandler_PacketIDs(t *testing.T) {
	// Verify packet IDs match expected values for 1.21.2 configuration phase
	tests := []struct {
		name       string
		packetFunc func() any
		expectedID int32
