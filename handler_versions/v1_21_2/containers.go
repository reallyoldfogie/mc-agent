...
func (c *containerHandler) SendContainerClick(conn models.PacketWriter, windowID int8, stateID, slot int32, button int8, mode int32, changedSlots map[int16]models.InventorySlot, carriedItem models.InventorySlot) error {
	...
	// Note: In 1.21.2, Slot structure uses ItemCount + UnnamedType0003 switch field
	...
	for slotNum, slotData := range changedSlots {
		...
		// Set the item data - in 1.21.2, Slot uses ItemCount and UnnamedType0003
		if slotData.Present {
...
// SendPickItem
// Note: In 1.21.2, the PickItem packet takes a block position, not a slot.
...
func (c *containerHandler) SendPickItem(conn models.PacketWriter, slot int32) error {
	// The 1.21.2 protocol changed PickItem to PickItemFromBlock which takes a position
	// instead of a slot. This interface needs to be redesigned for 1.21.2+.
...
// Note: 1.21.2+ includes the WorldBorderHit field (added in 1.21.2/protocol 768).
...
func convertSlotFromProtocol(slot basetypes.Slot) models.InventorySlot {
	...
	// The slot structure in 1.21.2 uses ItemCount > 0 to indicate presence
...
// In 1.21.2, the slot field is i8 (byte).
...
func (c *containerHandler) SendContainerButtonClick(conn models.PacketWriter, windowID int8, buttonID int8) error {
	...
	utils.SafeLogger(c.logger).Debug(fmt.Sprintf("[v1.21.2 Container] SendContainerButtonClick: windowID=%d buttonID=%d", windowID, buttonID))
